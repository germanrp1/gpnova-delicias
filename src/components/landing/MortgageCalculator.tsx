...
export const MortgageCalculator = ({
  ...
  const importePrestamo = purchasePrice - savings > 0 ? purchasePrice - savings : 0;

  // Cálculo de la cuota mensual utilizando la fórmula del sistema de amortización francés
  const calcularCuotaMensual = () => {
    if (importePrestamo <= 0 || interest <= 0 || years <= 0) return 0;
    const interesMensual = (interest / 100) / 12;
    const numeroPagos = years * 12; // 30 años * 12 = 360 meses
  ...
  const gastosEstimados = Math.round(purchasePrice * 0.10); 
  const dineroTotalNecesario = savings + gastosEstimados;
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-zinc-800 space-y-2 text-sm">
                    ...
                    <div className="flex justify-between border-t border-zinc-800 pt-2 font-medium">
                      ...
                      <span style={{ color: project.branding.primaryColor }}>{dineroTotalNecesario.toLocaleString()} €</span>
                    </div>
                  </div>
                </div>
